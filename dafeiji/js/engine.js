var Game = new function() {
  ...
  this.initialize = function(canvas_dom, level_data, sprite_data, callbacks) {
    Game.this.canvas_elem = $(canvas_dom)[0];
    // this.canvas_elem = document.getElementById("gameboard");
    this.canvas = this.canvas_elem.getContext('2d');
    this.width = $(this.canvas_elem).attr('width');
    this.height = $(this.canvas_elem).attr('height');

    $(window).keydown(function(event) {
      if (KEY_CODES[event.keyCode]) Game.keys[KEY_CODES[event.keyCode]] =
        true;
    });
...
var Sprites = new function() {
  ...
  this.draw = function(canvas, sprite, x, y, frame) {
    ...
    if (!frame) frame = 0;
    canvas.drawImage(this.image, s.sx + frame * s.w, s.sy, s.w, s.h, x, y,
      s.w, s.h);
  };
}
...
var GameScreen = function GameScreen(text, text2, callback) {
  ...
  this.render = function(canvas) {
    ...
    canvas.fillText(text, Game.width / 2 - measure.width / 2, Game.height /
      2);
    canvas.font = "bold 20px arial";
    var measure2 = canvas.measureText(text2);
    canvas.fillText(text2, Game.width / 2 - measure2.width / 2, Game.height /
      2 + 40);
  };
};
  ...
  this.play = function(s) {
    for (a = 0; a < audio_channels.length; a++) {
      ...
      if (audio_channels[a]['finished'] < thistime.getTime()) {
        audio_channels[a]['finished'] = thistime.getTime() + this.sounds[s]
          .duration * 1000;
